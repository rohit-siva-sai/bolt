      ...
      <section className="text-gray-600 body-font min-h-[80vh]">
        <div className="container lg:px-24 py-12 mx-auto ">
          <div className="flex flex-wrap -m-4 justify-center">
            {Object.keys(products).length===0 && <p className="mt-4">Sorry all the stickers are currently out of stock. New stock is coming soon. stay tuned</p> }
...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    if (item.title in stick) {
      if (
        !stick[item.title].color.includes(item.color) &&
        item.availableQty > 0
      ) {
        stick[item.title].color.push(item.color);
      }
      if (
        !stick[item.title].size.includes(item.size) &&
        item.availableQty > 0
      ) {
        stick[item.title].size.push(item.size);
      }
    } else {
      stick[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        stick[item.title].color = [item.color];
        stick[item.title].size = [item.size];
      }
    }
  }
  ...
  return {
    props: { products: JSON.parse(JSON.stringify(stick)) }, // will be passed to the page component as props
  };
}
